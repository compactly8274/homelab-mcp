...
- ``HOMELAB_MCP_OLLAMA_URL``         Ollama base URL (default
                                     ``http://127.0.0.1:11434``; used by
                                     the ollama_* tools and as a default
                                     conduit for the search tools)
...
- ``HOMELAB_MCP_OLLAMA_ALLOW_DELETE`` bool (default ``false``). When ``true``,
                                     ...
                                     the deployment.
- ``HOMELAB_MCP_OLLAMA_CLOUD_URL``     Direct Ollama Cloud API base URL
                                      (optional; e.g. ``https://api.ollama.com``).
                                      When set alongside
                                      ``HOMELAB_MCP_OLLAMA_CLOUD_API_KEY``,
                                      search tools send models ending in
                                      ``:cloud`` directly to this endpoint
                                      instead of through the local conduit.
- ``HOMELAB_MCP_OLLAMA_CLOUD_API_KEY`` API key/bearer token for direct Ollama
                                      Cloud calls (optional; keep secret)
...
- ``HOMELAB_MCP_SEARCH_DECOMPOSER_MODEL``  Ollama model used by
                                           ``deep_search`` to break the
                                           question into sub-queries.
                                           (default ``qwen3.5:cloud``)
- ``HOMELAB_MCP_SEARCH_CURATOR_MODEL``     Ollama model used by
                                           ``deep_search`` to curate
                                           evidence. (default
                                           ``deepseek-v4-flash:cloud``)
- ``HOMELAB_MCP_SEARCH_MAX_SUBQUERIES``    Maximum number of sub-queries
                                           generated by ``deep_search``.
                                           (default ``4``)
- ``HOMELAB_MCP_SEARCH_RESULTS_PER_SUBQUERY``  Results to keep per
                                               sub-query. (default ``5``)
- ``HOMELAB_MCP_SEARCH_FETCH_FULL_PAGES``  Whether ``deep_search`` and
                                           ``quick_search`` should fetch
                                           full page text. (default
                                           ``false``)
- ``HOMELAB_MCP_SEARCH_PAGE_TIMEOUT``      Page fetch timeout in seconds.
                                           (default ``8``)
"""
...
class Settings(BaseModel):
    ...
    ix_apps_root: str = "/mnt/.ix-apps/app_configs"
    ...
    # Ollama: local instance URL (often used as a conduit to Ollama Cloud).
    ollama_url: str = "http://127.0.0.1:11434"
    ollama_allow_pull: bool = False
    ollama_allow_delete: bool = False
    # Optional direct Ollama Cloud API access. If both URL and key are set,
    # models ending in ':cloud' bypass the local conduit and hit the cloud
    # API directly using the native /api/generate shape.
    ...
    search_max_subqueries: int = 4
    search_results_per_subquery: int = 5
    search_fetch_full_pages: bool = False
    search_page_timeout: int = 8
    ...
    def _from_env(cls, data: Any) -> dict[str, Any]:
        ...
        if "ix_apps_root" not in data:
            data["ix_apps_root"] = os.getenv(
                "HOMELAB_MCP_IX_APPS_ROOT", "/mnt/.ix-apps/app_configs"
            )
        ...
        if "ollama_url" not in data:
            data["ollama_url"] = os.getenv("HOMELAB_MCP_OLLAMA_URL", "http://127.0.0.1:11434")
        ...
        if "ollama_cloud_url" not in data:
        ...
        if "search_curator_model" not in data:
            data["search_curator_model"] = os.getenv(
                "HOMELAB_MCP_SEARCH_CURATOR_MODEL", "deepseek-v4-flash:cloud"
            )
        if "search_max_subqueries" not in data:
            data["search_max_subqueries"] = _env_int("HOMELAB_MCP_SEARCH_MAX_SUBQUERIES", 4)
        if "search_results_per_subquery" not in data:
            data["search_results_per_subquery"] = _env_int(
                "HOMELAB_MCP_SEARCH_RESULTS_PER_SUBQUERY", 5
            )
        if "search_fetch_full_pages" not in data:
            data["search_fetch_full_pages"] = _env_bool(
                "HOMELAB_MCP_SEARCH_FETCH_FULL_PAGES", False
            )
        if "search_page_timeout" not in data:
            data["search_page_timeout"] = _env_int("HOMELAB_MCP_SEARCH_PAGE_TIMEOUT", 8)
